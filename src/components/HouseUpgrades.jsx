...
const HouseUpgrades = () => {
  ...
  // Add new state for selected materials
  const [selectedMaterials, setSelectedMaterials] = useState({});

  // Add quantities to the state
  const [quantities, setQuantities] = useState({});

  const createNewHouse = () => {
    const houseNumber = selectedHouses.length + 1;
    const newHouse = {
      name: `House Upgrade ${houseNumber}`,
      materials: [],
    };
  ...
  useEffect(() => {
    ...
    const houseData = selectedHouses.map((house) => ({
      name: house.name,
      materials: selectedMaterials[house.name] || [],
      quantities: Object.keys(quantities)
        .filter((key) => key.startsWith(house.name))
        .reduce((acc, key) => {
    ...
    localStorage.setItem("houseUpgrades", JSON.stringify(houseData));
    ...
    <div className="flex-1 min-h-screen bg-white">
      <div className="p-6 w-full">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-2xl font-bold">House Upgrades</h2>
          <button
